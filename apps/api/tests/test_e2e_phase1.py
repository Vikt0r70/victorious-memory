...
status, resp = req("PUT", "/providers/extraction", {
    "role": "extraction",
...
status, resp = req("POST", "/projects/detect", {
    "path": "/workspace/my-test-project",
    "name": "Test Project"
})
...
for i in range(15):
    ...
    if status == 200 and resp.get("total", 0) > 0:
        ...
        print(f"     Worker processed in ~{(i+1)*2}s — {resp['total']} memories extracted")
        break
    if i == 14:
        ...
        for evt in activity.get("items", []):
            print(f"     Activity: [{evt['event_type']}] {evt['description']}")

check("At least 1 memory extracted", len(memories_found) > 0, f"got {len(memories_found)}")
...
status, ctx = req("GET", f"/context?project_id={project_id}&query=database&tokens=1500")
