...
const STATUSES = ["active", "pending_review", "deprecated", "superseded", "rejected"];
...
export default function EditMemoryModal({ memory, onClose, onSaved }: Props) {
  const [content, setContent] = useState(memory.content || "");
  const [memoryType, setMemoryType] = useState(memory.memory_type || "reference");
  const [scope, setScope] = useState(memory.scope || "global");
  const [status, setStatus] = useState(memory.status || "active");
  const [confidence, setConfidence] = useState(memory.confidence_score || 0.8);
