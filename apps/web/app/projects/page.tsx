  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-muted-foreground text-[14px] mt-1">Registered workspaces and their memory profiles</p>
      </div>
      {projects.length === 0 ? (
        <div className="bg-card border border-border rounded-lg p-12 text-center">
          ...
          <p className="text-muted-foreground">Projects are auto-detected when you use OpenCode in a workspace.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {projects.map((p, i) => (
            <Link key={p.id} href={`/projects/${p.id}`} className={`bg-card border border-border rounded-lg p-5 hover-glow stat-card-transition cursor-pointer fade-in-up delay-${(i + 1) * 100}`}>
              <div className="flex items-start justify-between mb-2">
                <h3 className="text-[18px] font-semibold text-foreground">{p.display_name}</h3>
                <div className="w-2 h-2 rounded-full bg-[#4ade80] mt-2" />
              </div>
              <div className="font-mono text-[12px] text-muted-foreground mb-3">{p.workspace_path}</div>
              {p.tech_stack && p.tech_stack.length > 0 && (
                <div className="flex gap-1 flex-wrap mb-3">
                  {p.tech_stack.map((t: string) => (
                    <span key={t} className="badge bg-accent border border-border text-muted-foreground">{t}</span>
                  ))}
                </div>
              )}
              ...
              <div className="mt-3 text-[13px] text-primary font-semibold flex items-center gap-1">
